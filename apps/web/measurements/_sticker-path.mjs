/**
 * THE STICKER AUDIT, NARROWED TO ONE ROUTE. A copy of sticker-audit.mjs with
 * a route filter and nothing else changed: every rule, threshold and detector
 * is the shared instrument's, byte for byte.
 *
 * WHY IT EXISTS. The shared audit walks every route in one process and throws
 * the moment a drive fails to reach its moment, which is right. In this tree
 * the reward route belongs to another piece that is mid-edit, so the shared
 * audit aborts on `reward-first` before it ever reaches #/pathway and the
 * pathway would have NO sticker number at all. This runs the same audit over
 * one route, named by ONLY. The whole-app total stays the shared audit's to
 * produce once that route lands; nothing here is a substitute for it.
 *
 * WALL CLOCKS: none added. The pathway route carries no wall-clock branch.
 */
 ...
 *
 * Writes measurements/_sticker-path.json and exits nonzero on any violation.
 */
...
await writeFile(
  path.resolve(process.cwd(), "measurements", "_sticker-path.json"),
  `${JSON.stringify(
    {
...
console.log(`wrote measurements/_sticker-path.json`);

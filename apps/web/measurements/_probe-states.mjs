...
console.log(JSON.stringify(await page.evaluate(()=>{
  const units=[...document.querySelectorAll(".path-unit")].slice(0,4).map(u=>({
    id:u.dataset.unitId,
    anchors:[...u.querySelectorAll("[data-trail]")].map(a=>`${a.dataset.trail}${a.dataset.trailDone==="true"?"+":"-"}${a.dataset.trailGate?"G":""}`),
    states:[...u.querySelectorAll(".path-node")].map(n=>[...n.classList].find(c=>c.startsWith("path-node--")&&c!=="path-node--press")),
  }));
  return units;
}),null,1));

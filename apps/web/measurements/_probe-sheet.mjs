/** Does pressing a node open the NODE sheet (not the charge sheet)? */
...
console.log(JSON.stringify(await page.evaluate(()=>({
  nodeSheetOpen: document.querySelector(".ns-sheet")?.hasAttribute("open") ?? false,
  hasPractice: [...document.querySelectorAll(".ns-panel h3")].map(h=>h.textContent),
  pips: document.querySelectorAll(".ns-panel [class*=pip], .ns-panel svg").length,
  hamburger: document.querySelector(".ns-menu") !== null,
  chargeSheetOpen: document.querySelector("dialog.charge-sheet")?.hasAttribute("open") ?? null,
})),null,1));
await page.screenshot({path: path.join(process.cwd(),"measurements","_shots","sheet.png")});
